...
class Learner(object):
    ...
    def __init__(self):
        ...
        self.Q = np.zeros((self.dim_vel, self.dim_td, self.dim_hd, 2))
        self.counts = np.zeros((self.dim_vel, self.dim_td, self.dim_hd, 2))
    ...
    def action_callback(self, state):
        ...
        new_action = 0 if self.Q[cv, ctd, chd, 0] >= self.Q[cv, ctd, chd, 1] else 1
        Q_max = self.Q[cv, ctd, chd, new_action]
        #eps = self.eps * (1 / ((self.counts[cv, ctd, chd, new_action]))) if self.counts[cv, ctd, chd, new_action] > 0 else self.eps
        eps = self.eps
        new_action = rand_action if rand_num < eps else new_action


        lr = 1 / (2 + self.counts[lv, ltd, lhd, self.last_action])
        #lr = 0.2 # fixed
        self.counts[cv, ctd, chd, new_action] += 1
        self.Q[lv, ltd, lhd, la] -= lr * (self.Q[lv, ltd, lhd, la] - (reward + self.gamma * Q_max))
...
def run_games(learner, hist, iters = 100, t_len = 100):
    ...
    plt.ylabel("Score")
    plt.savefig("Rel-lr=0.2-eps=0-gamma=0.9-line.png")
    plt.close
    ...
    plt.xlabel("Score")
    plt.ylabel("Counts")
    plt.savefig("Rel-lr=0.2-eps=0-gamma=0.9-hist.png")

    plt.figure()
    plt.plot(range(len(hist)), list(accumulate(hist)))
    plt.xlabel("Iterations")
    plt.ylabel("Running Score Sum")
    plt.savefig("Rel-lr=0.2-eps=adaptive-gamma=0.9-sum.png")
